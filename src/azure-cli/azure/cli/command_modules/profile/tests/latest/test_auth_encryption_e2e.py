...
Microsoft Graph application permissions Application.ReadWrite.All and User.ReadWrite.All granted to
that service principal. Without them the class skips rather than fails. The identities are granted
only the Reader role, and are deleted after the class; the configured service principal that
creates them is never deleted.
"""
...
class MultiIdentityCredentialScenarioTest(LiveScenarioTest):
    ...
    def _login_all(self, env):
        for identity in self.identities:
            _run_az(self._login_args(identity) + ['-o', 'none'], env=env)
    ...
    def _assert_no_credentials_remain(self, encrypted):
    ...
    def test_account_clear_with_encryption_off_clears_the_encrypted_store(self):
        """A clear must reach the OS credential store even when it is not the configured one.

        false -> true -> false leaves a payload in Keychain or libsecret while the setting says
        plaintext. There the file is only a modification signal, so removing it hides the payload
        instead of removing it, and turning the setting back on would make the credentials usable
        again. The service principal secrets are the ones that matter: the token cache is rewritten
        by the next login, but ServicePrincipalStore.save_entry reads the store with no signal file
        check, so stale secrets are loaded back and persisted again indefinitely.
        """
        self._skip_without_both_backends()
        plaintext_env = self._case_env(encrypt=False, dbus_broken=False)
        encrypted_env = self._case_env(encrypt=True, dbus_broken=False)
        ...
        # plaintext file, so the encrypted payload is the one a clear could overlook.
        self._assert_token_from_store(plaintext_env, 'the plaintext store did not serve a token')
        for name in (TOKEN_CACHE, SECRET_STORE):
            self.assertTrue(os.path.isfile(_path(name, file_extension_plaintext)))
            self.assertTrue(os.path.isfile(_path(name, _encrypted_extension())))

        # Phase 4: clear, with encryption off.
        ...
        # Turning encryption back on must not bring the phase 2 credentials back to life. This is
        # what fails when a clear only empties the backend it is configured for.
        self.assertFalse(self._logged_in_names(encrypted_env),
                         'turning encryption on after a clear brought accounts back')
        self._assert_no_token_from_store(
            encrypted_env, 'the encrypted credentials outlived a clear run with encryption off')
    ...
    def test_account_clear_with_encryption_off_is_idempotent(self):
        ...
        This is the run that has the most left to do and the most that can go wrong twice: with
        encryption off the clear reaches into the OS credential store as well as the plaintext
        file, so the second run asks Keychain or libsecret to overwrite a payload the first run
        already emptied, under a signal file that is no longer there.

        Neither run may raise or print the retry warning, and the second must not undo the first:
        it writes an empty payload through the same persistence, which touches the signal file, so
        a clear that stopped short of removing the file again would leave one behind pointing at
        ...
        for attempt in range(1, 3):
            ...
            # Checked after every run, so a second clear that resurrected or re-signalled a
            # credential is caught rather than hidden by the first run's success.
            self.assertFalse(self._logged_in_names(plaintext_env),
            ...
            self._assert_no_token_from_store(
                encrypted_env, f'an encrypted credential outlived clear #{attempt}')
    ...
    def test_switching_encryption_on_without_dbus_keeps_using_plaintext(self):
        ...
        warned = _run_az(self._login_args(self.identities[0]) + ['-o', 'none'], env=fallback_env)
